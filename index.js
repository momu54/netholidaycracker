...
app.on('ready', () => {
	...
	window.webContents.debugger.on('message', async (_event, method, params) => {
		...
		if (params.response.url.includes('result_list.action')) {
			const { body } = await window.webContents.debugger.sendCommand(
				'Network.getResponseBody',
				{ requestId: params.requestId }
			);
			console.log(body);
			/** @type {{title: string, id: number, answer: string}[]} */
			answers = [
				...new Set([
					...answers,
					.../** @type {any[]} */ (JSON.parse(body).data.list).map(
						(answer, index) => {
							return {
								topic: topicmap.get(index + 1),
								id: answer.id,
		...
		} else if (params.response.url.includes('answer_topic.action')) {
			...
			topicmap.set(data.title, data.topic);
		}
	});
...
async function WriteAnswer() {
	return await writeFile(`${answerspath}/answers.json`, JSON.stringify(answers));
}
